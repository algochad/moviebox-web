...
const DEV_SECRET = 'moviebox-account-dev-secret-please-override-in-production-7f3d9a1c';

export const AUTH_SECRET: string = process.env.AUTH_SECRET?.trim() || DEV_SECRET;
...
export const JWT_EXPIRES_IN = (process.env.AUTH_TOKEN_TTL?.trim() || '30d') as JwtExpiry;

export const DEFAULT_PORT = 4100;

export const PORT: number = Number(process.env.AUTH_PORT ?? process.env.PORT ?? DEFAULT_PORT);

export const HOST: string = process.env.AUTH_HOST?.trim() || '0.0.0.0';
